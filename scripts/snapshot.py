...
channels = [fetch(handle) for handle in CHANNELS]
successes = [c for c in channels if not c.get("error")]
failures = [c for c in channels if c.get("error")]
...
if failures:
    ...
    for channel in failures:
        print(f"- @{channel['handle']}: {channel['error']}")
